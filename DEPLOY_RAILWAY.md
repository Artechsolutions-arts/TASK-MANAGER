...
- **Start command must be `python start.py`:** The repo’s `backend/railway.toml` sets `startCommand = "python start.py"` so `PORT` is read from the environment (no `$PORT` shell expansion). If your deployment still uses `uvicorn ... --port $PORT`, either ensure Railway uses the config file (Settings → Config-as-code → path to `railway.toml`), or in **Settings → Deploy** set **Start command** to `python start.py` and save. The literal `$PORT` causes "Invalid value for '--port': '$PORT' is not a valid integer".

...
- **No custom Start command (or use `python start.py`):** In **Settings → Deploy**, leave **Custom Start Command** empty so the image’s `CMD` runs (`python start.py`, which reads `PORT` from the environment). If you have a start command that uses `--port $PORT`, remove it—`$PORT` is not expanded when Railway runs without a shell and causes "Invalid value for '--port': '$PORT'".
